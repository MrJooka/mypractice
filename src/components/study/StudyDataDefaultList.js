...
class StudyDataDefaultList extends Component {
  ...
  onFinish = (values) => {
    ...
    axios
      .post("api/studysetup/set-level-config", {
  ...
  getStudySetting = (value) => {
    ...
    axios
      .post("api/studysetup/get-level-config", {
        book_id: value,
      })
      .then((res) => {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: () => (
          <>
            <ArrowUpOutlined
              onClick={() => this.props.listOrderHandler({ action: "up", from: "like", category_id: this.props.bookInfo.category_id._id, bookId: this.props.bookInfo._id, seq_in_like: this.props.bookInfo.seq_in_like })}
              style={{ fontSize: "14px" }}
            />
            <ArrowDownOutlined
              onClick={() => this.props.listOrderHandler({ action: "down", from: "like", category_id: this.props.bookInfo.category_id._id, bookId: this.props.bookInfo._id, seq_in_like: this.props.bookInfo.seq_in_like })}
              style={{ fontSize: "14px" }}
            />
          </>
        ),
      ...
      },
      {
        ...
        render: (text, record) => {
          if (record) {
            return (
              <>
                <Button size="small" onClick={() => this.getStudyData(record.book_id)}>
                  상세보기
                </Button>
                <StudyData
                  studySetting={this.state.study_configuration}
                  handleOk={this.studyDataHandleOk}
                  book_status={this.state.book_status}
                  info={record}
                  isModalVisible={this.state.study_data_visible_array}
                  handleCancel={this.studyDataHandleCancel}
                />
              </>
            );
          }
      ...
      },
      {
        ...
        render: (text, record) => {
          if (record) {
            return (
              <>
                <Button size="small" onClick={() => this.getStudySetting(record.book_id)}>
                  학습설정
                </Button>
