...
class ListColumns extends Component {
  ...
  hideOrShowToggle = (checked) => {
    axios
      .post("api/book/change-hide-config", {
        hide_toggle: checked,
      })
      .then((res) => {
        this.props.updateHideOrShowState({
  ...
  render() {
    ...
    return (
      <ul className="like_list_columns" style={list_columns}>
        ...
        <li style={{ width: "80px", textAlign: "center" }}>
          숨긴책보기
          {this.props.hideOrShowToggleState === false ? (
            <>
              <Switch size="small" onChange={this.onChange} />
              {this.props.hideOrShowToggleState}
            </>
          ) : (
            <>
              <Switch size="small" checked onChange={this.onChange} />
              {this.props.hideOrShowToggleState}
            </>
          )}
        </li>
...
class ListContent extends Component {
  ...
  eyeClickHandler = (value) => {
    ...
    axios
      .post("api/book/change-hide-or-show", {
      ...
      .then((res) => {
        this.props.updateState({
          value1: res.data.categorybooklist,
          value2: res.data.likebooklist,
        });
      });
  };
  ...
  listOrderHandler = (value) => {
    ...
    if (value.from === "list") {
      axios
        .post("api/book/change-book-order", {
          book_id: value.bookId,
          action: value.action,
          seq_in_category: value.seq_in_category,
          category_id: value.category_id,
      ...
      axios
        .post("api/book/change-likebook-order", {
          book_id: value.bookId,
          action: value.action,
          seq_in_like: value.seq_in_like,
        })
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps.category !== this.state.category) {
      ...
      if (prevState.blinkId !== this.state.blinkId) {
        document.getElementById(this.state.blinkId).style.animation =
          "blinker 0.5s linear 1";
      }
    }
  }
          ...
          return (
            <StarTwoTone
              onClick={() =>
                this.onClickLike({
                  value: "true",
                  bookId: this.props.bookInfo._id,
                })
              }
              twoToneColor="#52c41a"
              style={{ fontSize: "14px" }}
      ...
      return (
        <div className="like_list_contents" id={`${this.props.book_id}`}>
          ...
          >
            ...
            >
              {this.state.editBookTitle ? (
                ...
                <>
                  <span
                    onClick={() =>
                      this.saveBookIdSession({ book_id: info._id })
                    }
                  >
            ...
            <li style={{ width: "50px", textAlign: "center" }}>
              <EditOutlined
                onClick={this.editBookTitleHandler}
                style={{ fontSize: "14px" }}
              />
            </li>
            ...
            <li style={{ width: "90px", textAlign: "center" }}>
              {info.num_cards.read.total}
            </li>
            <li style={{ width: "50px", textAlign: "center" }}>
              read {info.num_cards.read.total}장<br />
              flip {info.num_cards.flip.total}장
            </li>
            ...
            >
              <CategoryMoveModal
                updateState={this.props.updateState}
                category={this.props.categoryTotal}
                bookTitle={info}
              />
            </li>
            <li style={{ width: "70px", textAlign: "center" }}>
            ...
            <li style={{ width: "70px", textAlign: "center" }}>
              {info.hide_or_show === true ? (
                <>
                  <ArrowUpOutlined
                    onClick={() =>
                      this.listOrderHandler({
                        blinkId: this.props.book_id,
                        action: "up",
                        from: "list",
                        category_id: this.props.bookInfo.category_id._id,
                        bookId: this.props.bookInfo._id,
                        seq_in_category: this.props.bookInfo.seq_in_category,
                      })
                    }
                    style={{ fontSize: "14px" }}
                  />
                ...
                <EyeInvisibleOutlined
                  onClick={() =>
                    this.eyeClickHandler({
                      ...
                      bookId: this.props.bookInfo._id,
                      seq_in_category: this.props.bookInfo.seq_in_category,
                      seq_in_like: this.props.bookInfo.seq_in_like,
                      category_id: this.props.bookInfo.category_id._id,
                    })
                  }
                  style={{ fontSize: "14px" }}
            ...
            <li style={{ width: "30px", textAlign: "center" }}>
              <DeleteBook
                bookTitle={info}
                updateState={this.props.updateState}
              />
    ...
    return (
      <>
        {toggleProps === true ? (
          ...
          <>{info.hide_or_show === true ? elements() : ""}</>
        )}
      </>
    );
  }
...
class CategoryListContainer extends Component {
  ...
  render() {
    ...
    return (
      <div className="each_category_container" style={each_category_container}>
        {bookList}
      </div>
    );
  }
